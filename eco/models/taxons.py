...
class Taxon(Base, JsonifyMixin):
    ...
    def species_by_taxon(taxon_ids):
        ...
        taxons = dbsession.query(Taxon.id, Taxon.taxon_type, Taxon.name, Taxon.author,Taxon.source).from_statement(qs).all()
    ...
    def add_from_file(filename):
        ...
        for row in records:
            id, parent_id,old_id,taxon_type,name,russian_name,author,source = [None if x=='' else x  for x in row]
            taxon = Taxon(
                id = id,
                parent_id = parent_id, 
                old_id = old_id,
